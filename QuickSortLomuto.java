...
public class QuickSortLomuto {
    ...
    /*
     * Эта функция принимает последний элемент в качестве опорного элемента,
     * помещает этот элемент на свое место в массиве. А далее размещает элементы
     * ниже опоры - слева, а выше - справа.
     */
    static int partition(int[] arr, int low, int high) {
        ...
        for (int j = low; j <= high - 1; j++) {
            ...
            // Если текущий элемент меньше чем опроный элемент
            if (arr[j] < pivot) {
    ...
    static void quickSort(int[] arr, int low, int high) {
        if (low < high) {
            ...
            // pi - опроный элемент встает на своем место
